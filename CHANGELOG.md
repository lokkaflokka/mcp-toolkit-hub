...
- **`briefing_import_read_later`** — Replaced tab delimiter with `|||` to prevent breakage if titles contain tabs. Successfully imported items are now auto-completed in the Read Later list (title-only items remain open for URL resolution).
